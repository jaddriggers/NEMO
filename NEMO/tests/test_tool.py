...
class ToolTestCase(TestCase):
    ...
    def setUp(self):
        ...
        interlock_card_data = {
            "server": "example.com",
            "port": 25,
            "category": interlock_category.id,
        }
        interlock_card_form = InterlockCardAdminForm(interlock_card_data)
        self.assertTrue(
            interlock_card_form.is_valid(), interlock_card_form.errors.as_text()
        )
        ...
        area_door = Door.objects.create(
            name="cleanroom door", area=cleanroom, interlock=cleanroom_interlock
        )
        ...
        self.assertTrue(
            alternate_tool_form.is_valid(), alternate_tool_form.errors.as_text()
        )
    ...
    def test_tool_and_parent_properties(self):
        ...
        self.assertEquals(
            tool.notification_email_address, alternate_tool.notification_email_address
        )
        self.assertEquals(tool.interlock, alternate_tool.interlock)
        self.assertEquals(
            tool.requires_area_access, alternate_tool.requires_area_access
        )
        self.assertEquals(
        ...
        self.assertEquals(
            tool.minimum_usage_block_time, alternate_tool.minimum_usage_block_time
        )
        self.assertEquals(
            tool.maximum_usage_block_time, alternate_tool.maximum_usage_block_time
        )
        self.assertEquals(
            tool.maximum_reservations_per_day,
            alternate_tool.maximum_reservations_per_day,
        )
        self.assertEquals(
            tool.minimum_time_between_reservations,
            alternate_tool.minimum_time_between_reservations,
        )
        self.assertEquals(
            tool.maximum_future_reservation_time,
            alternate_tool.maximum_future_reservation_time,
        )
        self.assertEquals(
            tool.missed_reservation_threshold,
            alternate_tool.missed_reservation_threshold,
        )
        self.assertEquals(
            tool.allow_delayed_logoff, alternate_tool.allow_delayed_logoff
        )
        self.assertEquals(
            tool.post_usage_questions, alternate_tool.post_usage_questions
        )
        self.assertEquals(
            tool.policy_off_between_times, alternate_tool.policy_off_between_times
        )
        self.assertEquals(
            tool.policy_off_start_time, alternate_tool.policy_off_start_time
        )
    ...
    def test_tool_in_use(self):
        ...
        response = self.client.post(
            reverse("login_to_area", kwargs={"door_id": area_door.id}),
        ...
        self.assertEquals(
            tool.get_current_usage_event(), alternate_tool.get_current_usage_event()
        )
