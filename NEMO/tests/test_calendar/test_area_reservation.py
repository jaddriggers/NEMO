...
class AreaReservationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        area = Area.objects.create(
            name="test_area", requires_reservation=True, category="Imaging"
        )
        ...
        staff = User.objects.create(
            username="staff", first_name="Staff", last_name="Member", is_staff=True
        )
    ...
    def test_user_does_not_meet_conditions(self):
        user = User.objects.create(
            username="noproj", first_name="scott", last_name="NoProj"
        )
        ...
        self.assertContains(
            response,
            "You are blocked from making reservations in the NanoFab. Please complete the NanoFab rules tutorial in order to create new reservations.",
        )
        self.assertContains(
            response,
    ...
    def test_reservation_policy_problems(self):
        # start tomorrow 2am
        dt_now = datetime.now()
        base_start = datetime(dt_now.year, dt_now.month, dt_now.day) + timedelta(
            days=1, hours=2
        )
        ...
        self.assertEqual(
            ...
            "The request parameters have an end time that precedes the start time.",
        )
        ...
        outage = ScheduledOutage.objects.create(
            title="Outage", area=area, start=base_start, end=end, creator=staff
        )
        data = self.get_reservation_data(base_start, end, area)
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        data = self.get_reservation_data(
            base_start - timedelta(hours=1), end - timedelta(minutes=59), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            base_start + timedelta(minutes=59), end + timedelta(hours=1), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            base_start - timedelta(days=1, hours=2), end - timedelta(days=1), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
        ...
        area.save()
        data = self.get_reservation_data(
            start + timedelta(hours=2), end + timedelta(hours=2), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        area.save()
        data = self.get_reservation_data(
            start + timedelta(minutes=90), end + timedelta(minutes=90), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            start + timedelta(minutes=30), start + timedelta(minutes=90), area
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
    ...
    def test_create_reservation_multi_projects(self):
        ...
        second_project = Project.objects.create(
            name="project2", account=Account.objects.get(name="account1")
        )
        ...
        not_my_project = Project.objects.create(
            name="not my project", account=Account.objects.get(name="account1")
        )
        data["project_id"] = not_my_project.id
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Reservation.objects.all().count(), 0)
        expected_exception = NotAllowedToChargeProjectException(
            not_my_project, consumer
        )
    ...
    def test_resize_reservation(self):
        ...
        response = self.client.post(
            reverse("resize_reservation"),
            {"delta": "asd", "id": reservation.id},
            follow=True,
        )
        self.assertEqual(response.status_code, 400)
        ...
        response = self.client.post(
            reverse("resize_reservation"), {"delta": 10}, follow=True
        )
        ...
        ScheduledOutage.objects.create(
            area=area, start=start_reservation, end=end_reservation, creator=staff
        )
        response = self.client.post(
            reverse("resize_reservation"),
            {"delta": 61, "id": reservation.id},
            follow=True,
        )
        ...
        self.assertEquals(
            Reservation.objects.filter(area=area, cancelled=False).count(), 1
        )
        new_reservation = list(Reservation.objects.filter(area=area, cancelled=False))[
            0
        ]
        self.assertEquals(
            new_reservation.end, old_reservation.end - timedelta(minutes=10)
        )
        ...
        self.assertContains(
            response, "This reservation has already been cancelled", status_code=400
        )

        # test increase reservation time by 10 min
        response = self.client.post(
            reverse("resize_reservation"),
            {"delta": 10, "id": new_reservation.id},
            follow=True,
        )
        self.assertEquals(response.status_code, 200)
        ...
        AreaAccessRecord.objects.create(
            area=area, customer=consumer, start=datetime.now(), project=project
        )
        response = self.client.post(
            reverse("resize_reservation"),
            {"delta": 10, "id": older_resa_still_ongoing.id},
            follow=True,
        )
        self.assertEquals(response.status_code, 200)
    ...
    def test_move_reservation(self):
        ...
        # test no reservation id
        response = self.client.post(
            reverse("move_reservation"), {"delta": 10}, follow=True
        )
        ...
        self.assertEquals(
            new_reservation.start, old_reservation.start - timedelta(minutes=10)
        )
    ...
    def test_cancel_reservation(self):
        ...
        response = self.client.get(
            reverse("cancel_reservation", kwargs={"reservation_id": 999}),
            {},
            follow=True,
        )
        ...
        response = self.client.post(
            reverse("cancel_reservation", kwargs={"reservation_id": reservation.id}),
            {},
            follow=True,
        )
        self.assertEquals(response.status_code, 400)
        ...
        response = self.client.post(
            reverse("cancel_reservation", kwargs={"reservation_id": missed_resa.id}),
            {},
            follow=True,
        )
        self.assertEquals(response.status_code, 400)
        ...
        response = self.client.post(
            reverse(
                "cancel_reservation", kwargs={"reservation_id": already_ended_resa.id}
            ),
            {},
            follow=True,
        )
        ...
        self.assertContains(
            response, "This reservation has already been cancelled by ", status_code=400
        )
        ...
        login_as(self.client, staff)
        response = self.client.post(
            reverse("cancel_reservation", kwargs={"reservation_id": other_resa.id}),
            {},
            follow=True,
        )
        self.assertContains(
    ...
    def test_reservation_details(self):
        ...
        # post should fail
        response = self.client.post(
            reverse("reservation_details", kwargs={"reservation_id": 999}),
            {},
            follow=True,
        )
        self.assertEquals(response.status_code, 405)
        ...
        response = self.client.get(
            reverse("reservation_details", kwargs={"reservation_id": reservation.id}),
            {},
            follow=True,
        )
        self.assertEquals(response.status_code, 200)
