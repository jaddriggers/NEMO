...
class ReservationTestCase(TestCase):
    ...
    def test_reservation_policy_problems(self):
        ...
        outage = ScheduledOutage.objects.create(
            title="Outage", tool=tool, start=base_start, end=end, creator=staff
        )
        data = self.get_reservation_data(base_start, end, tool)
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        data = self.get_reservation_data(
            base_start - timedelta(hours=1), end - timedelta(minutes=59), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            base_start + timedelta(minutes=59), end + timedelta(hours=1), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            base_start - timedelta(days=1, hours=2), end - timedelta(days=1), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
        ...
        self.assertContains(
            response,
            "You may not create reservations further than 2 days from now for this tool.",
        )
        ...
        tool.save()
        data = self.get_reservation_data(
            start + timedelta(hours=2), end + timedelta(hours=2), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        tool.save()
        data = self.get_reservation_data(
            start + timedelta(minutes=90), end + timedelta(minutes=90), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
        ...
        data = self.get_reservation_data(
            start + timedelta(minutes=30), start + timedelta(minutes=90), tool
        )
        response = self.client.post(reverse("create_reservation"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(
    ...
    def test_resize_reservation(self):
        ...
        ScheduledOutage.objects.create(
            tool=tool, start=start_reservation, end=end_reservation, creator=staff
        )
        response = self.client.post(
            reverse("resize_reservation"),
        ...
        self.assertEquals(
            Reservation.objects.filter(tool=tool, cancelled=False).count(), 1
        )
        new_reservation = list(Reservation.objects.filter(tool=tool, cancelled=False))[
            0
        ]
        self.assertEquals(
