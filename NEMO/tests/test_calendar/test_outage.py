...
class OutageTestCase(TestCase):
    ...
    def outage_policy_problems(self, item_id: int, item_type: ReservationItemType):
        ...
        data = self.get_outage_data(
            start=start, end=end, item_id=item_id, item_type=item_type
        )
    ...
    def test_create_outage(self):
        ...
        data = self.get_outage_data(
            title="Outage", start=start, end=end, item_id=tool.id
        )
        login_as_staff(self.client)
        response = self.client.post(reverse("create_outage"), data, follow=True)
    ...
    def resize_outage(self, item_id: int, item_type: ReservationItemType):
        ...
        data = self.get_outage_data(
            title="Outage", start=start, end=end, item_id=item_id, item_type=item_type
        )
        login_as_staff(self.client)
        response = self.client.post(reverse("create_outage"), data, follow=True)
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(
            reverse("resize_outage"), {"delta": "asd", "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        # test no outage id
        response = self.client.post(
            reverse("resize_outage"), {"delta": 10}, follow=True
        )
        ...
        response = self.client.post(
            reverse("resize_outage"), {"delta": -60, "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 400)
        ...
        response = self.client.post(
            reverse("resize_outage"), {"delta": 61, "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.content.decode(),
        ...
        response = self.client.post(
            reverse("resize_outage"), {"delta": -10, "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
    ...
    def move_outage(self, item_id: int, item_type: ReservationItemType):
        ...
        # test wrong delta
        response = self.client.post(
            reverse("move_outage"), {"delta": "asd", "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.content.decode(), "Invalid delta")
        ...
        response = self.client.post(
            reverse("move_outage"), {"delta": 61, "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.content.decode(),
        ...
        response = self.client.post(
            reverse("move_outage"), {"delta": -10, "id": outage.id}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            ScheduledOutage.objects.get(pk=outage.id).end,
    ...
    def cancel_outage(self, item_id: int, item_type: ReservationItemType):
        ...
        response = self.client.get(
            reverse("cancel_outage", kwargs={"outage_id": 999}), {}, follow=True
        )
        test_response_is_landing_page(self, response)
        self.assertTrue(ScheduledOutage.objects.get(pk=outage.id).id, outage.id)
        login_as_staff(self.client)
        ...
        response = self.client.post(
            reverse("cancel_outage", kwargs={"outage_id": outage.id}), {}, follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(ScheduledOutage.objects.all().count(), 0)
    ...
    def outage_details(self, item_id: int, item_type: ReservationItemType):
        ...
        # post should fail
        response = self.client.post(
            reverse("outage_details", kwargs={"outage_id": 999}), {}, follow=True
        )
        self.assertEqual(response.status_code, 405)
        ...
        response = self.client.get(
            reverse("outage_details", kwargs={"outage_id": outage.id}), {}, follow=True
        )
        self.assertEqual(response.status_code, 200)
    ...
    def every_day_for_a_week(self, item_id: int, item_type: ReservationItemType):
        ...
        outages = ScheduledOutage.objects.filter(
            title="every day outage week", tool=tool
        )
    ...
    def every_week_for_a_year(self, item_id: int, item_type: ReservationItemType):
        start = (
            (datetime.now() + timedelta(days=1)).replace(microsecond=0).replace(hour=7)
        )
        end = start + timedelta(hours=1)
    ...
    def week_day(self, item_id: int, item_type: ReservationItemType):
        ...
        self.assertEqual(response.status_code, 200)
        outages = ScheduledOutage.objects.filter(
            title="every week day outage", tool=tool
        )
        for outage in outages:
            # 0 is Monday, 5 & 6 are Saturday and Sunday
            self.assertLess(
                outage.start.astimezone(timezone.get_current_timezone()).weekday(), 5
            )
