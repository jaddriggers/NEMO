...
def mocked_requests_get(*args, **kwargs):
    ...
    if (
        args[0]
        == f"{bad_interlock.card.server}:{bad_interlock.card.port}/stateFull.xml?relay3State=1"
    ):
        return MockResponse("", 500, UNLOCK_ERROR)
    elif (
        args[0]
        == f"{bad_interlock.card.server}:{bad_interlock.card.port}/stateFull.xml?relay3State=0"
    ):
        return MockResponse("", 500, LOCK_ERROR)
    # interlock 2 on server 2 sends wrong response
    elif (
        args[0]
        == f"{wrong_response_interlock.card.server}:{wrong_response_interlock.card.port}/stateFull.xml?relay2State=0"
        or args[0]
        == f"{wrong_response_interlock.card.server}:{wrong_response_interlock.card.port}/stateFull.xml?relay2State=1"
    ):
        return MockResponse("bad response", 200)
    # interlock 5 on server 2 sends wrong state response
    elif (
        args[0]
        == f"{wrong_state_interlock.card.server}:{wrong_state_interlock.card.port}/stateFull.xml?relay2State=0"
        or args[0]
        == f"{wrong_state_interlock.card.server}:{wrong_state_interlock.card.port}/stateFull.xml?relay5State=1"
    ):
...
class WebRelayInterlockTestCase(TestCase):
    ...
    def setUp(self):
        ...
        wrong_state_interlock = Interlock.objects.create(
            card=interlock_card2, channel=5
        )
        ...
        tool = Tool.objects.create(
            name="test_tool", primary_owner=owner, interlock=interlock
        )
    ...
    def test_disabled_card(self, mock_args):
        ...
        self.assertTrue(
            "Interlock interface mocked out" in disabled_interlock.most_recent_reply
        )
    ...
    def test_wrong_response_from_interlock(self, mock_args):
        ...
        self.assertTrue(
            "General exception" in wrong_response_interlock.most_recent_reply
        )
    ...
    def test_wrong_state_from_interlock(self, mock_args):
        ...
        self.assertTrue(
            "Unexpected state received" in wrong_state_interlock.most_recent_reply
        )
