...
class BillingAPITestCase(TestCase):
    def setUp(self):
        ...
        owner1 = User.objects.create(
            username="mctest1", first_name="Testy", last_name="McTester"
        )
        tool1 = Tool.objects.create(name="test_tool1", primary_owner=owner1)
        owner2 = User.objects.create(
            username="mctest2", first_name="Testy", last_name="McTester"
        )
        tool2 = Tool.objects.create(name="test_tool2", primary_owner=owner2)

        account = Account.objects.create(name="Test Account")
        project = Project.objects.create(
            name="Test Project", account=account, application_identifier="N19.0001"
        )
    ...
    def billing_by_attribute(
        self, attribute_name, attribute_value, result_attribute_name, results_number
    ):
        data = {
        ...
        self.assertEqual(
            response.status_code, 403, "regular user or staff doesn't have permission"
        )
