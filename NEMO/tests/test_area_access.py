...
class AreaAccessGetTestCase(TestCase):
    ...
    def test_area_access_page_by_user(self):
        ...
        test_response_is_landing_page(
            self, response
        )  # since user is not staff, it should redirect to landing
...
class KioskAreaAccess(TestCase):
    ...
    def setUp(self):
        ...
        area = Area.objects.create(
            name="Cleanroom", welcome_message="Welcome to the cleanroom"
        )
        door = Door.objects.create(name="test_door", area=area, interlock=interlock)

    def test_welcome_screen_fails(self):
        response = self.client.post(
            reverse("welcome_screen", kwargs={"door_id": door.id}), follow=True
        )
        test_response_is_failed_login(self, response)
        login_as_user(self.client)
        response = self.client.post(
        ...
        test_response_is_landing_page(
            self, response
        )  # landing since we don't have the right credentials
        ...
        response = self.client.get(
            reverse("welcome_screen", kwargs={"door_id": 999}), follow=True
        )
    ...
    def test_farewell_screen_fails(self):
        response = self.client.post(
            reverse("farewell_screen", kwargs={"door_id": door.id}), follow=True
        )
        test_response_is_failed_login(self, response)
        login_as_user(self.client)
        response = self.client.post(
        ...
        self.assertEqual(response.status_code, 405)  # POST isn't accepted, only GET
        response = self.client.get(
            reverse("farewell_screen", kwargs={"door_id": 999}), follow=True
        )
        self.assertEqual(response.status_code, 404)  # wrong door id
    ...
    def test_login_to_area(self):
        response = self.client.post(
            reverse("login_to_area", kwargs={"door_id": door.id}), follow=True
        )
        test_response_is_failed_login(self, response)
        login_as_user(self.client)
        response = self.client.post(
        ...
        response = self.client.post(
            reverse("login_to_area", kwargs={"door_id": 999}), follow=True
        )
        self.assertEqual(response.status_code, 404)  # wrong door id
        response = self.client.post(
            reverse("login_to_area", kwargs={"door_id": door.id}), follow=True
        ...
        self.assertContains(
            response, f"The {door.area} has reached its maximum capacity."
        )
        ...
        self.assertTrue(f"login_to_area/{door.id}" in response.request["PATH_INFO"])
        self.assertContains(
            response=response, text="You're logged in to the ", status_code=200
        )
        self.assertTrue(
            AreaAccessRecord.objects.filter(
                area=door.area,
                customer=User.objects.get(badge_number=staff.badge_number),
            ).exists()
        )
        ...
        self.assertTrue(
            AreaAccessRecord.objects.filter(
                area=door.area,
                customer=User.objects.get(badge_number=user.badge_number),
            ).exists()
        )

    def test_staff_login_to_area(self):
        staff = login_as_staff(self.client)
        tablet_user = login_as_user_with_permissions(
            self.client, ["add_areaaccessrecord"]
        )
        response = self.client.post(
            reverse("login_to_area", kwargs={"door_id": door.id}),
            data={"badge_number": staff.badge_number},
        ...
        self.assertTrue(
            "already logged into" in str(response.content)
        ...
        test_response_is_landing_page(
            self, response
        )  # tablet user does not have permission to logout
...
class SelfLoginAreaAccessTestCase(TestCase):
    ...
    def test_login_to_area(self):
        ...
        Customization.objects.update_or_create(
            name="self_log_in", defaults={"value": "enabled"}
        )
        self.assertFalse(
            AreaAccessRecord.objects.filter(area=area, end__isnull=True).exists()
        )
        ...
        self.client.post(
            reverse("self_log_in"),
            data={"area": area.id, "project": project.id},
            follow=True,
        )
        self.assertFalse(
            AreaAccessRecord.objects.filter(area=area, end__isnull=True).exists()
        ...
        self.assertTrue(
            AreaAccessRecord.objects.filter(
                area=area, customer=user, end__isnull=True
            ).exists()
        )
    ...
    def test_staff_login_to_area(self):
        ...
        self.assertEqual(
            AreaAccessRecord.objects.filter(
                area=area, customer=staff, end__isnull=True
            ).count(),
            1,
        )
        self.assertEqual(
        ...
        # logout
        Customization.objects.update_or_create(
            name="self_log_out", defaults={"value": "enabled"}
        )
        self.client.get(
            reverse("self_log_out", kwargs={"user_id": staff.id}), follow=True
        )
...
class NewAreaAccessTestCase(TestCase):
    ...
    def test_new_area_record_get(self):
        user = login_as_user(self.client)
        response = self.client.get(
            reverse("new_area_access_record"), data={"customer": user.id}, follow=True
        )
